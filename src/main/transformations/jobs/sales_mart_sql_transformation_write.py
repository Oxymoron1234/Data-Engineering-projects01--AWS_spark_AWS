...
#Caculate customer mart
#find out customer total purchase every month write the data into mysql table

def sales_mart_calculation_table_write(final_sales_team_data_mart_df):
    window = Window.partitionBy("store_id","sales_person_id","sales_month","sales_year")
    ...
    rank_window = Window.partitionBy("store_id","sales_month").orderBy("total_sales_every_month")
    ...
    data_writer = DatabaseWriter(config.url,config.properties)
    data_writer.write_dataframe(final_sales_team_data_mart_table,config.sales_team_data_mart_table)
